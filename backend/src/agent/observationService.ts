...
export interface ObservationResult {
  outcome: 'RECOVERED' | 'STILL_OPEN' | 'ACTION_FAILED' | 'MAX_ATTEMPTS_REACHED' | 'WAITING' | 'TERMINAL';
  reason: string;
  shouldContinue: boolean;
  recommendedNextStep: 'NONE' | 'RETRY_RECOVERY' | 'ESCALATE_TO_HUMAN' | 'CLOSE_NO_ACTION' | 'WAIT';
  previousAction?: string;
}

const TERMINAL_STATUSES = ['RECOVERED', 'CLOSED_NO_RECOVERY', 'ESCALATED'];

export async function observeCaseOutcome(caseId: string): Promise<ObservationResult> {
  ...
  // RULE 6: Contact Cooldown Protection
    ...
    if (elapsed < cooldownMs) {
      ...
      const allowed = getAllowedActions(caseRecord.type, caseRecord.attemptCount);
