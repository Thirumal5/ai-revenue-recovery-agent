...
async function runPhase8TestSuite() {
  ...
  const obs1 = await observeCaseOutcome(case1.id);
  ...
  const obs2 = await observeCaseOutcome(case2.id);
  ...
  const obs3 = await observeCaseOutcome(case3.id);
  assert(obs3.outcome === 'ACTION_FAILED', 'Test 3: Outcome marked ACTION_FAILED');
  const checkCase3 = await prisma.recoveryCase.findUnique({ where: { id: case3.id } });
  ...
  const obs4 = await observeCaseOutcome(case4.id);
  ...
  const obs5 = await observeCaseOutcome(case5.id);
  ...
  const obs6 = await observeCaseOutcome(case6.id);
  ...
  const obsAction = await prisma.agentAction.findFirst({
  ...
  const res9 = await processCase(case8.id);
  const updatedCase9 = await prisma.recoveryCase.findUnique({ where: { id: case8.id } });
  ...
  await observeCaseOutcome(case11.id);
  const check11After = await prisma.recoveryCase.findUnique({ where: { id: case11.id } });
  ...
  const res12 = await processCase(case12.id);
