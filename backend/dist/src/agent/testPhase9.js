...
function logTrace(scenario, testCase, event, caseId, subReason, allowed, decision, safety, tool, apiType, observation, finalStatus) {
    console.log(`\n--------------------------------------------------`);
...
async function runPhase9TestSuite() {
    ...
    const case1A = await prisma_1.prisma.recoveryCase.create({
        data: {
    ...
    const res1A = await (0, orchestrator_1.processCase)(case1A.id);
    const updated1A = await prisma_1.prisma.recoveryCase.findUnique({ where: { id: case1A.id } });
    ...
    logTrace('Payment Failure', 'Normal Recovery', event1A, case1A.id, updated1A?.subReason || 'insufficient_funds', ['SEND_PAYMENT_LINK', 'SEND_REMINDER', 'SUGGEST_ALTERNATIVE_PAYMENT', 'ESCALATE_TO_HUMAN'], 'SEND_PAYMENT_LINK', true, 'SEND_PAYMENT_LINK', 'REAL RAZORPAY TEST API', updated1A?.observationOutcome || 'STILL_OPEN', updated1A?.status || 'OPEN');
    // 1B: Technical Tool Failure Protection
    const case1B = await prisma_1.prisma.recoveryCase.create({
        data: {
            customerId: customer.id,
    ...
    const obs1B = await (0, observationService_1.observeCaseOutcome)(case1B.id);
    assert(obs1B.outcome === 'ACTION_FAILED', 'S1.B: Observation correctly identified ACTION_FAILED');
    const updated1B = await prisma_1.prisma.recoveryCase.findUnique({ where: { id: case1B.id } });
    assert(updated1B?.status !== 'RECOVERED', 'S1.B: Failed tool did NOT mark case RECOVERED');
    // 1C: Maximum Attempt Limit Enforcement
    const case1C = await prisma_1.prisma.recoveryCase.create({
        data: {
            customerId: customer.id,
            type: 'payment_failure',
            amount: 7500,
    ...
    process.env.COOLDOWN_MS = '0';
    await (0, orchestrator_1.processCase)(case1C.id);
    const updated1C = await prisma_1.prisma.recoveryCase.findUnique({ where: { id: case1C.id } });
    ...
    const case1D = await prisma_1.prisma.recoveryCase.create({
        data: {
            customerId: customer.id,
            type: 'payment_failure',
            amount: 1200,
    ...
    const res1D = await (0, orchestrator_1.processCase)(case1D.id);
    assert(res1D.success === false, 'S1.D: Processing rejected due to active contact cooldown');
    // 1E: Terminal State Protection
    const case1E = await prisma_1.prisma.recoveryCase.create({
        data: {
            customerId: customer.id,
            type: 'payment_failure',
            amount: 3000,
    ...
    const obs1E = await (0, observationService_1.observeCaseOutcome)(case1E.id);
    ...
    const case1F = await prisma_1.prisma.recoveryCase.create({
        data: {
            customerId: customer.id,
            type: 'payment_failure',
            amount: 0,
    ...
    process.env.COOLDOWN_MS = '0';
    const res1F = await (0, orchestrator_1.processCase)(case1F.id);
    ...
    const linkRes1G = await (0, toolDispatcher_1.executeTool)('SEND_PAYMENT_LINK', { id: case1A.id, amount: 4999, razorpayPaymentLinkId: updated1A?.razorpayPaymentLinkId }, { name: customer.name, email: customer.email }, { chosen_action: 'SEND_PAYMENT_LINK' });
    ...
    const case2A = await prisma_1.prisma.recoveryCase.create({
        data: {
            customerId: customer.id,
            type: event2A.type,
    ...
    process.env.COOLDOWN_MS = '0';
    const res2A = await (0, orchestrator_1.processCase)(case2A.id);
    const updated2A = await prisma_1.prisma.recoveryCase.findUnique({ where: { id: case2A.id } });
    ...
    logTrace('Checkout Abandonment', 'Normal Recovery', event2A, case2A.id, updated2A?.subReason || 'unknown_reason', ['SEND_REMINDER', 'SEND_PAYMENT_LINK', 'CLOSE_NO_ACTION'], 'SEND_REMINDER', true, 'SEND_REMINDER', 'SIMULATED INTERNAL ACTION', updated2A?.observationOutcome || 'STILL_OPEN', updated2A?.status || 'OPEN');
    // 2B: Technical Failure Handling
    const case2B = await prisma_1.prisma.recoveryCase.create({
        data: {
            customerId: customer.id,
    ...
    const obs2B = await (0, observationService_1.observeCaseOutcome)(case2B.id);
    ...
    await prisma_1.prisma.recoveryCase.update({ where: { id: case2A.id }, data: { attemptCount: 2, lastContactedAt: null } });
    process.env.COOLDOWN_MS = '0';
    await (0, orchestrator_1.processCase)(case2A.id);
    const updated2C = await prisma_1.prisma.recoveryCase.findUnique({ where: { id: case2A.id } });
    ...
    process.env.COOLDOWN_MS = '120000';
    const case2D = await prisma_1.prisma.recoveryCase.create({
        data: {
            customerId: customer.id,
            type: 'checkout_abandonment',
            amount: 2100,
    ...
    const res2D = await (0, orchestrator_1.processCase)(case2D.id);
    assert(res2D.success === false, 'S2.D: Abandonment processing blocked by active cooldown');
    // 2E: Terminal State Protection
    const case2E = await prisma_1.prisma.recoveryCase.create({
        data: { customerId: customer.id, type: 'checkout_abandonment', amount: 800, status: 'CLOSED_NO_RECOVERY', riskReason: 'Closed cart' },
    });
    const obs2E = await (0, observationService_1.observeCaseOutcome)(case2E.id);
    assert(obs2E.outcome === 'TERMINAL', 'S2.E: Closed cart state protected');
    // 2F: Invalid Event Handling
    const case2F = await prisma_1.prisma.recoveryCase.create({
        data: { customerId: customer.id, type: 'checkout_abandonment', amount: -50, status: 'OPEN', riskReason: 'Negative amount cart' },
    });
    process.env.COOLDOWN_MS = '0';
    const res2F = await (0, orchestrator_1.processCase)(case2F.id);
    assert(res2F.success === true, 'S2.F: Negative amount cart handled without crashing');
    // 2G: Idempotent Execution Verification
    const case2G = await prisma_1.prisma.recoveryCase.create({
        data: { customerId: customer.id, type: 'checkout_abandonment', amount: 1500, status: 'OPEN', riskReason: 'Duplicate abandonment check' },
    });
    process.env.COOLDOWN_MS = '0';
    const exec2G_1 = await (0, orchestrator_1.processCase)(case2G.id);
    process.env.COOLDOWN_MS = '120000';
    const updatedCase2G = await prisma_1.prisma.recoveryCase.findUnique({ where: { id: case2G.id } });
    const safetyCheck2G = (0, safetyEngine_1.checkSafetyRules)({ lockedForProcessing: false, lastContactedAt: updatedCase2G?.lastContactedAt || new Date(), status: 'OPEN', attemptCount: 1, type: 'checkout_abandonment' }, { chosen_action: 'SEND_REMINDER' }, ['SEND_REMINDER', 'SEND_PAYMENT_LINK', 'CLOSE_NO_ACTION']);
    ...
    const case3A = await prisma_1.prisma.recoveryCase.create({
        data: {
            customerId: customer.id,
            type: event3A.type,
    ...
    process.env.COOLDOWN_MS = '0';
    const res3A = await (0, orchestrator_1.processCase)(case3A.id);
    const updated3A = await prisma_1.prisma.recoveryCase.findUnique({ where: { id: case3A.id } });
    ...
    logTrace('Subscription Failure', 'Normal Recovery', event3A, case3A.id, updated3A?.subReason || 'card_expired', ['SEND_CARD_UPDATE_REMINDER', 'SEND_PAYMENT_LINK', 'SUGGEST_ALTERNATIVE_PAYMENT', 'ESCALATE_TO_HUMAN'], 'SEND_CARD_UPDATE_REMINDER', true, 'SEND_CARD_UPDATE_REMINDER', 'SIMULATED INTERNAL ACTION', updated3A?.observationOutcome || 'STILL_OPEN', updated3A?.status || 'OPEN');
    // 3B: Tool Failure Handling
    const case3B = await prisma_1.prisma.recoveryCase.create({
    ...
    const obs3B = await (0, observationService_1.observeCaseOutcome)(case3B.id);
    ...
    const case3C = await prisma_1.prisma.recoveryCase.create({
    ...
    process.env.COOLDOWN_MS = '0';
    await (0, orchestrator_1.processCase)(case3C.id);
    const updated3C = await prisma_1.prisma.recoveryCase.findUnique({ where: { id: case3C.id } });
    assert(updated3C?.status === 'ESCALATED', 'S3.C: Subscription case escalated after 3 attempts');
    // 3D: Contact Cooldown Enforcement
    process.env.COOLDOWN_MS = '120000';
    const case3D = await prisma_1.prisma.recoveryCase.create({
    ...
    process.env.COOLDOWN_MS = '0';
    const res3F = await (0, orchestrator_1.processCase)(case3F.id);
    ...
    const case3G = await prisma_1.prisma.recoveryCase.create({
        data: { customerId: customer.id, type: 'subscription_failure', amount: 2999, status: 'OPEN', riskReason: 'Duplicate sub event' },
    });
    process.env.COOLDOWN_MS = '0';
    const exec3G_1 = await (0, orchestrator_1.processCase)(case3G.id);
    process.env.COOLDOWN_MS = '120000';
    const updatedCase3G = await prisma_1.prisma.recoveryCase.findUnique({ where: { id: case3G.id } });
    const safetyCheck3G = (0, safetyEngine_1.checkSafetyRules)({ lockedForProcessing: false, lastContactedAt: updatedCase3G?.lastContactedAt || new Date(), status: 'OPEN', attemptCount: 1, type: 'subscription_failure' }, { chosen_action: 'SEND_CARD_UPDATE_REMINDER' }, ['SUGGEST_ALTERNATIVE_PAYMENT', 'SEND_REMINDER', 'SEND_PAYMENT_LINK', 'ESCALATE_TO_HUMAN']);
    ...
    const case4A = await prisma_1.prisma.recoveryCase.create({
        data: {
            customerId: customer.id,
            type: event4A.type,
    ...
    process.env.COOLDOWN_MS = '0';
    const res4A = await (0, orchestrator_1.processCase)(case4A.id);
    const updated4A = await prisma_1.prisma.recoveryCase.findUnique({ where: { id: case4A.id } });
    ...
    logTrace('Overdue Invoice', 'Normal Recovery', event4A, case4A.id, updated4A?.subReason || 'unknown_reason', ['SEND_REMINDER', 'SEND_PAYMENT_LINK', 'SUGGEST_ALTERNATIVE_PAYMENT', 'ESCALATE_TO_HUMAN'], 'SEND_REMINDER', true, 'SEND_REMINDER', apiType4A, updated4A?.observationOutcome || 'STILL_OPEN', updated4A?.status || 'OPEN');
    // 4B: Tool Failure Handling
    const case4B = await prisma_1.prisma.recoveryCase.create({
    ...
    const obs4B = await (0, observationService_1.observeCaseOutcome)(case4B.id);
    assert(obs4B.outcome === 'ACTION_FAILED', 'S4.B: Invoice tool failure observed');
    // 4C: Maximum Attempt Limit Enforcement
    const case4C = await prisma_1.prisma.recoveryCase.create({
    ...
    process.env.COOLDOWN_MS = '0';
    await (0, orchestrator_1.processCase)(case4C.id);
    const updated4C = await prisma_1.prisma.recoveryCase.findUnique({ where: { id: case4C.id } });
    assert(updated4C?.status === 'ESCALATED', 'S4.C: Invoice case escalated after 3 attempts');
    // 4D: Contact Cooldown Restriction
    process.env.COOLDOWN_MS = '120000';
    const case4D = await prisma_1.prisma.recoveryCase.create({
