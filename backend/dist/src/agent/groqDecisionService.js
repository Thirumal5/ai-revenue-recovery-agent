...
function getFallbackDecision(context, reasoning) {
    ...
    if (context.allowedActions && context.allowedActions.length > 1 && context.priorActionsSummary) {
        const untried = context.allowedActions.find((act) => act !== 'ESCALATE_TO_HUMAN' && !context.priorActionsSummary.includes(act));
        if (untried) {
...
async function decideRecoveryAction(context) {
    ...
    for (const modelName of candidateModels) {
        try {
            ...
            if (!context.allowedActions.includes(decision.chosen_action)) {
                ...
                return getFallbackDecision(context, `AI chose "${decision.chosen_action}" which is not in the allowed actions list`);
            }
