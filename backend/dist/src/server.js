...
const scheduler_1 = require("./agent/scheduler");
...
app.get('/api/analytics', async (req, res) => {
    try {
        const cases = await prisma_1.prisma.recoveryCase.findMany({
            include: { actions: true },
        });
        const totalCases = cases.length;
...
app.post('/webhooks/simulator', async (req, res) => {
    ...
    try {
        ...
        const recoveryCase = await prisma_1.prisma.recoveryCase.create({
            data: {
        ...
        (0, orchestrator_1.processCase)(recoveryCase.id).catch((err) => {
...
app.post('/api/cases/:id/process', async (req, res) => {
    try {
        ...
        const result = await (0, orchestrator_1.processCase)(req.params.id);
        res.json(result);
    }
    catch (error) {
